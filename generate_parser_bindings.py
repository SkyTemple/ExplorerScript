...
def generate_bindings(classes, visitor_methods):
    ...
    for class_name, methods in classes.items():
        ...
        bindings.append(f'    .def(py::init<antlr4::ParserRuleContext*, size_t>())')
        ...
        for method_name, overloads in grouped_methods.items():
            if len(overloads) > 1:
                for method in overloads:
                    if method['params']:
                        bindings.append(f'    .def("{method_name}", py::overload_cast<{method["params"].split(' ')[0]}>(&{class_name}::{method_name}))')
                    else:
    ...
    bindings.append('    .def("visitChildren", [](ExplorerScriptVisitor& self, antlr4::tree::ParseTree* node) {') # todo
...
def generate_trampoline_class(visitor_methods):
    ...
    trampoline_class.append(f'    std::any defaultResult() override {{')
    trampoline_class.append(f'        PYBIND11_OVERRIDE_PURE(')
    trampoline_class.append(f'            pybind11::object,') # Return type
    trampoline_class.append(f'            ExplorerScriptVisitor,') # Parent class
    trampoline_class.append(f'            defaultResult')
    trampoline_class.append(f'        );')
    trampoline_class.append(f'    }}')
